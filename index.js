...
bot.start(async (ctx) => {
    ...
    await ctx.reply(
        `🍒 <b>Cherry Deals | Fast & Safety</b> - Ваш надежный партнер в безопасных сделках!\n\n` +
        `<b>Почему клиенты выбирают нас:</b>\n\n` +
        `🔒 <b>Гарантия безопасности</b> - все сделки защищены\n` +
        `💎 <b>Мгновенные выплаты</b> - в любой валюте\n` +
        `🛡 <b>Круглосуточная поддержка</b> - решаем любые вопросы\n` +
...
const mainServer = require('http').createServer((req, res) => {
    if (req.url === '/health' || req.url === '/') {
        ...
        res.end(JSON.stringify({ 
            ...
            service: 'CherryDeals Bot',
            timestamp: new Date().toISOString(),
            message: 'Бот работает и готов принимать команды'
        }));
    } else {
        res.writeHead(404);
        res.end('Not Found');
...
// Сервер для авторизации API
const authServer = require('http').createServer(app);
authServer.listen(AUTH_PORT, '0.0.0.0', () => {
    console.log(`🔐 Auth server listening on port ${AUTH_PORT}`);
});

// Запускаем бота с polling (самый надежный способ на Render)
bot.launch({
    ...
    console.log('🤖 CherryDeals Bot started with polling!');
    console.log('✅ Бот готов принимать команды: /start, /auth, /cherryteam, /ban, /unban, /vladelec');
    console.log('✅ Текстовые команды: Создать сделку, Мои сделки, Баланс, Авторизация');
}).catch((error) => {
...
console.log('🔄 Starting CherryDeals Bot...');
