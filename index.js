...
// Подтверждение передачи товара продавцом (С ОБРАБОТКОЙ ОШИБОК)
bot.action(/confirm_transfer_(.+)/, async (ctx) => {
    ...
    // Уведомление покупателю С ОБРАБОТКОЙ ОШИБОК
    try {
        ...
        // Сообщаем продавцу, что покупатель недоступен
        try {
            await ctx.telegram.sendMessage(
                deal.seller,
                `ℹ️ <b>Покупатель недоступен для уведомлений</b>\n\n` +
                `Мы не смогли уведомить покупателя о передаче товара.`,
                { parse_mode: 'HTML' }
            );
        } catch (e) {
            console.log('⚠️ Не удалось уведомить продавца о недоступности покупателя');
        }
    }
});

// Подтверждение получения товара покупателем (С ОБРАБОТКОЙ ОШИБОК)
bot.action(/confirm_receipt_(.+)/, async (ctx) => {
    ...
    // Уведомление продавцу С ОБРАБОТКОЙ ОШИБОК
    try {
        await ctx.telegram.sendMessage(
            deal.seller,
